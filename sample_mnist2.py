...
def sample_single_bootstrap_scenario3(distribution, file_index, train_labels, test_labels, cure_probs_dict):
    '''
        Get a single bootstrap sample from the Fashion-MNIST dataset considering each distribution from scenario 2.
    '''
    filename = "data_{}.csv".format(file_index)
    directory = "SimulationDataset2/Scenario3"
    
    # ---------------------------- Load the sample indices ----------------------------
    indices_img = pd.read_csv("SimulationDataset2/Scenario3/indices_{}.csv".format(file_index))
    i_train_val_sampled = indices_img["index"][ (indices_img["set"] == "train") | (indices_img["set"] == "val") ].to_numpy()
    i_test_sampled = indices_img["index"][ indices_img["set"] == "test" ].to_numpy()
...
def sample_bootstrap_scenario3(distribution, train_labels, test_labels, cure_probs_dict):
    print("Sampling distribution {}".format(distribution))
    for j in tqdm(range(1, 101)):
        sample_single_bootstrap_scenario3(distribution, j, train_labels, test_labels, cure_probs_dict)
