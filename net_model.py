...
class MPScrModelStructure(keras.models.Model):    
    ...
    def loss_func(self, m, eta):
        ...
        if(self.r_min is not None and self.r_max is not None):          
            ...
            r_min_vector = tf.constant(self.r_min, dtype = tf.float64, shape = (m.shape[0], 1))
            ...
            # Penalização para valores de p abaixo de p_0_min
            r_max_vector = tf.constant(self.r_max, dtype = tf.float64, shape = (m.shape[0], 1))
        ...
        # Notive that if the proper loss weights is positive, it is probabily greater than the loss related to the parametric space, because the probability values will be closer to
    ...
    def compile_model(self, learning_rate = 0.001, run_eagerly = False):
        self.compile(
...
def update_omega(model, m_r, x,
    ...
    new_model = model
    new_model.compile_model(learning_rate, run_eagerly)
